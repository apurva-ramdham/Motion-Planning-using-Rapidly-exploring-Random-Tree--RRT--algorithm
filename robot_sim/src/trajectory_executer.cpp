...
void TrajectoryExecuter::callback(const trajectory_msgs::JointTrajectory::ConstPtr &msg)
{
  ...
  assert(current_trajectory_.joint_names.size() == robot_->getNumJoints());
...
{
  std::vector<double> current_vals = robot_->getJointValues(current_trajectory_.joint_names);
  ...
  robot_->setVelocities(current_trajectory_.joint_names, current_vels_);
}
