...
int main(int argc, char **argv)
{
  ...
  ROS_INFO("Simulated robot running");

  bool cycle;
  priv_nh.param<bool>("cycle_move", cycle, false);
  ros::NodeHandle root_nh("");
  ros::Timer timer;
      if (cycle) {
	timer = root_nh.createTimer(ros::Duration(1.0),
				    boost::bind(setVelocities, robot, num_joints));
	ROS_INFO("Simulated robot move cycle started");
  }
